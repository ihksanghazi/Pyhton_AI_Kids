...
while True:
    ...
    if face_mesh_results.multi_face_landmarks:
        ...
        for face_num, face_landmarks in enumerate(face_mesh_results.multi_face_landmarks):
            if left_eye_status[face_num] == 'OPEN':
                frame = detector.masking(frame, left_eye, face_landmarks,'LEFT EYE', 
			                            detector.mpFaceMesh.FACEMESH_LEFT_EYE)
            if right_eye_status[face_num] == 'OPEN': 
                frame = detector.masking(frame, right_eye, face_landmarks, 'RIGHT EYE',
    ...
    # cv2.imshow(""left_eye_frame)
    # cv2.imshow(right_eye)
    # cv2.imshow("Face Mesh", frame_face_mesh)
    # cv2.imshow("Smoke Animation",smoke_frame)
